...
async def chat(req: ChatRequest, tenant: TokenData = Depends(get_current_tenant)):
    ...
    # Carga system message y funciones
    ...
    vsid = supabase.table("tenants")\
        ...
        .eq("id", tid)\
        .single().execute().data["openai_vector_store_id"]

    # Inicializa cliente OpenAI
    nokey = get_tenant_openai_key(tid)
    client = AsyncOpenAI(api_key=nokey)

    # Prepara mensajes y herramientas (incluye RAG)
    ...
    # Primera llamada RAG con API v1.x
    try:
        ...
        logger.error(f"Error RAG OpenAI: {e}")
        raise HTTPException(status_code=400, detail=e._message)

    choice = resp1.choices[0].message
    text = choice.content or ""
    fc = choice.function_call if choice.function_call else None

    # Si hubo llamada a función, ejecútala y prepara segunda iteración
    if fc:
        ...
        if missing:
            ...
            msgs.append({"role": "tool", "tool_call_id": fc.id, "name": fc.name, "content": json.dumps(resfn)})
    ...
    # Agregar enlace crítico si existe
